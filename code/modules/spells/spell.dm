	...
	switch(max_targets)
		...
		if(1) //single target can be picked
			if(range < 0)
				...
				//targets += browser_input_list("Choose the target for the spell.", "Targeting", possible_targets)
				//Adds a safety check post-input to make sure those targets are actually in range.
				var/mob/M
				if(!random_target)
					M = browser_input_list("Choose the target for the spell.", "Targeting", sortNames(possible_targets))
				else
