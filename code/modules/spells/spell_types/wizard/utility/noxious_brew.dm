	...
	if(con)
		if(con.spillable)
			if(fillable_volume > 0)
				con.reagents.add_reagent_list(list(main_ingredient = fillable_volume*0.6, secondary_ingredient = fillable_volume*0.3, tertiary_ingredient = fillable_volume*0.1))
