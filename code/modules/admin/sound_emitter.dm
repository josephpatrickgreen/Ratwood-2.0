...
/obj/effect/sound_emitter/Topic(href, href_list)
	...
	if(href_list["edit_mode"])
		...
		new_mode = browser_input_list(user, "Choose a new mode.", "Sound Emitter", mode_list)
		if(!new_mode)
			return
	...
	if(href_list["edit_range"])
		...
		new_range = browser_input_list(user, "Choose a new range.", "Sound Emitter", range_list)
		if(!new_range)
			return
