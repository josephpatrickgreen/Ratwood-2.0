...
/datum/team/proc/admin_add_objective(mob/user)
	...
	var/selected_type = browser_input_list("Select objective type:", "Objective type", GLOB.admin_objective_list)
...
/datum/team/proc/admin_add_member(mob/user)
	...
	for(var/mob/M in GLOB.mob_list)
		if(M.mind)
			minds |= M.mind
	var/datum/mind/value = browser_input_list("Select new member:", "New team member", null, sortNames(minds))
	if (!value)
		return
...
/datum/team/custom/proc/admin_force_hud(mob/user)
	...
	var/new_hud_state = browser_input_list(user,"Choose hud icon state","Custom HUD","traitor", sortList(possible_icons))
	if(!new_hud_state)
		return
